            ...
            children: [
                ...
                AnimatedOpacity(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.circular(12),
                      border: Border.all(color: AppColors.cautionAmber.withOpacity(0.5)),
                    ),
                    child: Row(
                      children: [
                        const Icon(Icons.warning_amber_rounded, color: AppColors.cautionAmber),
                        const SizedBox(width: 10),
                        const Expanded(
                          child: Text(
                            "PrescriptSense provides general health information only based on available dataset and internet's source. "
                        ...
                        IconButton(
                          icon: const Icon(Icons.close, color: AppColors.cautionAmber, size: 20),
                          onPressed: () {
                            if (!mounted) return;
                            setState(() {
                              _showDisclaimer = false;
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    suffixIcon: IconButton(
                      icon: Icon(
                        _isListening ? Icons.mic : Icons.mic_none,
                        color: _isListening ? AppColors.alertRed : AppColors.teal,
                      ),
              ...
              Align(
                ...
                child: ElevatedButton.icon(
                  onPressed: _isLoading ? null : () => sendToGemini(_problemController.text),
                  icon: const Icon(Icons.send),
                  label: Text(_isLoading ? "Sending..." : "Send"),
              ...
              if (_isLoading) ...[
                const SizedBox(height: 16),
                const Center(child: CircularProgressIndicator(color: AppColors.lavenderBlue)),
              ],
              ...
              if (_geminiResponse.isNotEmpty && !_isLoading) ...[
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(14),
                    border: Border.all(
                      color: _hasError ? AppColors.alertRed : AppColors.lavenderBlue,
                    ),
                  ),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Row(
                        ...
                        children: [
                          ...
                          IconButton(
                            icon: Icon(Icons.volume_up_rounded, color: _hasError ? AppColors.alertRed : AppColors.lavenderBlue),
                            onPressed: _speakResponse,
                      ...
                      Text(
                        _geminiResponse,
                        style: const TextStyle(fontSize: 15, height: 1.6, color: AppColors.slate),
                      ),
                    ],
                  ),
                ),
              ...
              GridView.count(
                ...
                children: [
                  _DashboardTile(
                    icon: Icons.upload_file,
                    label: 'Upload Prescription',
                    onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const PrescriptionUploadPage())),
                  ),
                  _DashboardTile(
                    icon: Icons.library_books,
                    label: 'Medicine Database',
                    onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const MedicineListPage())),
                  ),
                  _DashboardTile(
                    icon: Icons.medical_services,
                    label: 'Medicine Checker',
                    onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const MedicineCheckerPage())),
                  ),
                  _DashboardTile(
                    icon: Icons.record_voice_over,
                    label: 'Text to Speech',
                    onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const HandwrittenToTextPage())),
                  ),
                  _DashboardTile(
                    icon: Icons.alarm,
                    label: 'Medicine Reminder',
                    onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const ReminderListPage())),
                  ),
                  _DashboardTile(
                    icon: Icons.local_hospital,
                    label: 'Nearby Clinics',
                    onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const FindNearbyClinicPage())),
                  ),
                  _DashboardTile(
                    icon: Icons.history,
                    label: 'Prescription History',
                    onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const PrescriptionResultPage(imagePath: 'assets/sample_prescription.png'))),
                  ),
                ],
              ),
            ],
